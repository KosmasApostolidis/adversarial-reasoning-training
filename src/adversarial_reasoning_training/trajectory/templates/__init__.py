...
Each VLM family (Qwen2.5-VL, LLaVA-NeXT, LLaVA-OneVision, InternVL2)
has its own chat formatting and image-token expansion rules. The public
``assemble_*`` functions here turn a (prompt, trajectory, image,
processor) tuple into a :class:`TeacherForcedBatch` that the trainer
can feed back into ``forward_with_logits``.
...
External callers import the four public ``assemble_*`` symbols
...
from ._llava_onevision import assemble_llava_onevision
