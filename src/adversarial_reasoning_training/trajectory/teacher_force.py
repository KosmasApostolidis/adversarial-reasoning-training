...
def assemble(
    ...
    * ``qwen_vl`` → :func:`assemble_qwen` — ``processor`` is a Qwen2.5-VL
      ``AutoProcessor`` (carries ``.tokenizer`` + ``.image_processor``).
    * ``llava_next`` → :func:`assemble_llava_next` — ``processor`` is a
      ``LlavaNextProcessor``.
    * ``internvl2`` → :func:`assemble_internvl` — ``processor`` is the
      attacks-repo ``InternVL2`` wrapper itself (carries ``.tokenizer``,
